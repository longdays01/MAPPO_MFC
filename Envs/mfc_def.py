...
class EdgeDevice(object):
    ...
    def __init__(self, obs_r, pos, spd, collect_r, max_buffer_size, movable=True, mv_en=0, trans_bt=0):  # pos(x,y,h)
...
col0 = collecting_params[2]
col1 = collecting_params[3]
carrier_f = 2.5e9
p_v = 0.5 # vehicle transmit power 0.5W
p_r = 5 # RSU transmit power 5W
...
def collecting_rate(sensor, agent):
    ...
    L = Pl * col0 + col1 * (1 - Pl)
    gamma = p_v / (L * sensor.noise_power**2)
    rate = sensor.bandwidth * np.log2(1 + gamma)
    return rate


def data_collecting(sensors, agent, inzone_time):
    ...
    if agent.idle and (len(agent.total_data.keys()) < agent.max_buffer_size):
        ...
        if data_properties:
            inzone_time = max(data_properties)
            # print([data_properties, inzone_time])
            return inzone_time
        else:
            return 0
    # finish collection
...
def trans_rate(dist, agent, sensor):  # to be completed
    d = np.linalg.norm(np.array(sensor.position) - np.array(agent.position))
    B = 1e6 * agent.action.bandwidth
    d_hb = 4 #antenna height
    sigma_r_square = sigma_v_square = 10**(-13) # Gaussian noise
    L = 40 * (1 - 4 * 10**(-3) * d_hb) * np.log10(d) - 18 * np.log10(d_hb) + 21 * np.log10(carrier_f) + 80
    rate = B * np.log2(1 + p_v * 10**((-L) / 10)/sigma_v_square)
    print('agent-{} rate: {},{},{},{},{}'.format(agent.no, dist, agent.action.bandwidth, L, rate))
    return rate


class MFC_world(object):
    def __init__(self, map_size, agent_num, sensor_num, obs_r, speed, collect_r, max_size=1, sensor_lam=0.5):
        ...
        self.inzone_list = [0] * self.agent_count
    ...
    def step(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            # collecting
            self.inzone_list[i] = data_collecting(self.sensors, agent, self.inzone_list[i])
            # print(self.inzone_list[i])
        for id in age_dict.keys():
            self.sensor_age[id] = sorted(age_dict[id])[0]
        print('inzone:{}'.format(self.inzone_list))