...
# class MfcEnv(object):
#     ground_length = ground_width = 800
#     zone_length = 40
#     zone_width = 10
#     B = 10 ** 6 # bandwidth 1MHz
#     p_v = 0.5 # vehicle transmit power 0.5W
#     p_r = 5 # RSU transmit power 5W
#     sigma_r_square = sigma_v_square = 10 ** (-13) # Gaussian noise
#     f = 2800 # carrier frequency
#     comp_cycles = 1200 # number of computation cycles needed to execute 1 bit of data
#     delta_o = delta_d = 10 # SNR offloading and delivering conditions
#     lambda0 = 50 # per-unit penalty
#     epsilon = 1 # length of a time slot
#     d_hb = 4 #antenna height
#     c_r = 1.2e9 # RSU computing capability 
#     sum_task_size = 100 * 1048576 # Total computing task 60 Mbits --> 60 80 100 120 140
#     a = list(range(1, 12)) # index of the road
#     b = list(range(1, 10)) # index of segment on the road
#     z = [a,b] #index of zone z on the road
#     r = list(range(1, 24)) # set of RSU
#     t = [] # t-th time slot 
#     alpha_z_r_t = [] # offloading desision to Receiver RSU
#     beta_z_r_t = [] # offloading decision to Deliver RSU

#     def __init__(self):
#         self.start_state = np.append(self.start_state, self.sum_task_size)

class MFC_MARL_ENV(gym.Env):
    metadata = {
    ...
    def __init__(self, world, alpha=0.5, beta=0.2, aggregate_reward=False, discrete=True,
        ...
        for agent in self.agents:
            if self.discrete_flag:
                act_space = spaces.Tuple((onehot_space.OneHot(self.max_buffer_size), sum_space.SumOne(self.agent_num), onehot_space.OneHot(self.max_buffer_size)))
